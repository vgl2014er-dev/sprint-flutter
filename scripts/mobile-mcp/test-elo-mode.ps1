...
Invoke-MobileTapByLabel -Device $Device -LabelContains 'Elo Matches' -Output $Output -DryRun:$DryRun
Assert-MobileLabelVisible -Device $Device -LabelContains 'Generate Elo Matches' -DryRun:$DryRun
Assert-MobileLabelVisible -Device $Device -LabelContains 'Elo ' -DryRun:$DryRun

Invoke-MobileTapByLabel -Device $Device -LabelContains 'Generate Elo Matches' -Output $Output -DryRun:$DryRun
...
Write-Host ''
if ($DryRun) {
    Write-Host 'Elo mode test dry-run complete.'
} else {
    Write-Host 'Elo mode test passed.'
}

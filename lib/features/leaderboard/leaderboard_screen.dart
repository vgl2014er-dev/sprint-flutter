...
import '../../ui/widgets/conic_gradient_progress.dart';
...
class LeaderboardScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final derived = _LeaderboardDerivedCache.resolve(
      state.players,
      state.history,
    );
    final sortedPlayers = derived.sortedPlayers;
...
class _LeaderboardDerivedData {
  const _LeaderboardDerivedData({
    required this.sortedPlayers,
    required this.highlightedPlayerIds,
    required this.latestEloDeltaByPlayerId,
  });

  final List<Player> sortedPlayers;
  final Set<String> highlightedPlayerIds;
  final Map<String, int> latestEloDeltaByPlayerId;
}

class _LeaderboardDerivedCache {
  static List<Player>? _playersRef;
  static List<MatchHistoryEntry>? _historyRef;
  static _LeaderboardDerivedData? _cached;

  static _LeaderboardDerivedData resolve(
    List<Player> players,
    List<MatchHistoryEntry> history,
  ) {
    final cached = _cached;
    if (cached != null &&
        identical(players, _playersRef) &&
        identical(history, _historyRef)) {
      return cached;
    }
    ...
    if (latestMatch != null) {
      highlightedPlayerIds.add(latestMatch.p1Id);
      highlightedPlayerIds.add(latestMatch.p2Id);
      latestEloDeltaByPlayerId[latestMatch.p1Id] =
          latestMatch.p1EloAfter - latestMatch.p1EloBefore;
      latestEloDeltaByPlayerId[latestMatch.p2Id] =
          latestMatch.p2EloAfter - latestMatch.p2EloBefore;
    }

    final computed = _LeaderboardDerivedData(
      sortedPlayers: sortedPlayers,
      highlightedPlayerIds: highlightedPlayerIds,
      latestEloDeltaByPlayerId: latestEloDeltaByPlayerId,
    );
    _playersRef = players;
    _historyRef = history;
    _cached = computed;

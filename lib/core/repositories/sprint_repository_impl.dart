...
import '../logic/elo_engine.dart';
import '../logic/history_policy.dart';
import '../logic/match_rollback_engine.dart';
...
class SprintRepositoryImpl implements SprintRepository {
  ...
  Future<void> submitRoundResults(List<models.RoundResultInput> results) async {
    ...
    await _database.transactionRun(() async {
      ...
      final changedPlayersById = <String, models.Player>{};
      final insertedHistoryEntries = <models.MatchHistoryEntry>[];
      ...
      for (var index = 0; index < results.length; index += 1) {
        ...
        changedPlayersById[newP1.id] = newP1;
        changedPlayersById[newP2.id] = newP2;

        final insertedEntry = models.MatchHistoryEntry(
        ...
        existingHistory.add(insertedEntry);
        insertedHistoryEntries.add(insertedEntry);
      }

      if (!hasAppliedResults) {
        return;
      }

      updatedPlayers = playersById.values.toList(growable: false);
      updatedHistory = HistoryPolicy.cap(existingHistory);
      final retainedHistoryIds = updatedHistory
          .map((entry) => entry.id)
          .toList(growable: false);

      await _database.upsertPlayers(
        changedPlayersById.values
            .map(_playerToCompanion)
            .toList(growable: false),
      );
      await _database.upsertHistory(
        insertedHistoryEntries.map(_historyToCompanion).toList(growable: false),
      );
      await _database.deleteHistoryExceptIds(retainedHistoryIds);
    });
    ...
    _playersSubscription = playersRef.onValue.listen(
      (event) async {
        ...
        await _database.transactionRun(() async {
          ...
          await _database.deletePlayersExceptIds(
            remotePlayers.map((player) => player.id).toList(growable: false),
          );
        });
    ...
    _historySubscription = historyRef.onValue.listen(
      (event) async {
        ...
        await _database.transactionRun(() async {
          ...
          await _database.deleteHistoryExceptIds(
            remoteHistory.map((entry) => entry.id).toList(growable: false),
          );
        });
